...
for i, outpath in enumerate(localdspaths):
    ...
    for hcenter in rsad['halos','particle_position'].to('unitary'):
        r = ds.quan(r_big, 'kpc')
        ...
        try:
            if hsp['new_p3_stars','age'].size > 0:
                ...
                for j, c in enumerate(hsp['new_p3_stars','particle_position'].to('unitary')):
                    ...
                    pidx = int(sp['new_p3_stars','particle_index'][j])
                    ...
                    for dd in out_dumps:
                        ...
                        if os.path.exists(dsfile):
                            try:
                                ...
                                ds.add_field(('gas','sum_metallicity'), 
                                                function=_sum_metallicity, 
                                                units = 'Zsun', 
                                                sampling_type='cell')
                                ds.add_field(('gas','p3_metallicity'), 
                                                function=_p3_metallicity, 
                                                units = 'Zsun', 
                                                sampling_type='cell')
                                if dd == dnum:
                            ...
                            zmean = 1e-5 # critical metallicity, once mean is below this, the edge is found
                            zmax = 0
                            z = 10
                            nohigh=False # is there metallicity here?
                            while z >= zmean:
                                r *= 1.25
                                sp = dsn.sphere(c, r)
                                z = sp.quantities.weighted_average_quantity(('gas','sum_metallicity'), 
                                                                            ('gas','cell_volume'))
                                if z < zmean:
                                    zmean = float(z)
                                if z > zmax: 
                                ...
                                if r.to('kpc') > r_big:
                                    nohigh=True
                                    print('[%d] Breaking on RD%04d high-radii: %0.2f'%(rank, dnum, r.to('kpc')))
                                    break
                            if zmax < 1e-5: # no region had critical metallicity
                                nohigh=True | nohigh
                                print('[%d] Breaking from RD%04d on low averaged Z'%(rank, dnum))
                            if nohigh==True:
                                continue
                            ...
                            for field in profile_fields:
                                ...
                                z_max = z_prof[radius.to('kpccm') < 0.5].mean()
                                zcut = -1
                                rcut = -1
